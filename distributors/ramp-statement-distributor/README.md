...
A Python tool for the Apache Software Foundation Treasury that generates and distributes Ramp credit card activity statements to departments via email.

## Overview

This tool loads departments from `AccountGroups.json`, queries the Ramp database for transactions, generates CSV statements per department, and emails them to department contacts. All departments receive an email each month; departments with no transactions receive a no-activity notice (no attachment).
...
**Filename format:** `Ramp-{department}-{from_date}-{to_date}.csv`

## Ramp-Specific Troubleshooting

### Account Group Not Found / Empty CSV

- Department names and GL ranges are loaded from `packages/shared-utils/src/AccountGroups.json`
